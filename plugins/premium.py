...
async def premium_1min(bot, update):
    await add_premium_user(bot, update, timedelta(minutes=1), "1 Min", 0)

@Client.on_callback_query(filters.regex('premium_1day'))
async def premium_1day(bot, update):
    await add_premium_user(bot, update, timedelta(days=1), "1 Day", 10)

@Client.on_callback_query(filters.regex('premium_1month'))
async def premium_1month(bot, update):
    await add_premium_user(bot, update, timedelta(days=30), "1 Month", 90)

@Client.on_callback_query(filters.regex('premium_3months'))
async def premium_3months(bot, update):
    await add_premium_user(bot, update, timedelta(days=90), "3 Months", 150)

@Client.on_callback_query(filters.regex('premium_6months'))
async def premium_6months(bot, update):
    await add_premium_user(bot, update, timedelta(days=180), "6 Months", 350)

@Client.on_callback_query(filters.regex('premium_1year'))
async def premium_1year(bot, update):
    await add_premium_user(bot, update, timedelta(days=365), "1 Year", 500)

async def add_premium_user(bot, update, duration, plan_name, price):
    ...
    # Notify the admin
    await update.message.edit(f"Added {plan_name} Premium for {user_id} (Expires on: {expiry_date.strftime('%Y-%m-%d %H:%M:%S')})")
...
async def check_expired_premium(bot):
    ...
    for user in premium_users:
        if user['expiry_date'] <= datetime.now():
            await bot.send_message(user['user_id'], "Your premium membership has expired. Please renew to continue enjoying premium features.")
